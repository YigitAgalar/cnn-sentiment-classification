...
df_tfidf_clean = pd.read_csv("data/roberta_clean.csv",index_col=0)
...
df_tfidf_clean = max_score_extraction(df_tfidf_clean,"roberta_clean")
st.title('Bitcoin hakkında atılan tweetlerin sentiment analizi')

st.subheader('Roberta Transformer ile Sentiment Analizi')
...
#show the dataset
st.write(df_tfidf_clean[["roberta_neg_clean","roberta_neu_clean","roberta_pos_clean","roberta_clean_max_score","sentiment"]].sample(10))
...
st.dataframe(allscores[allscores.index.str.startswith("rb")])


st.image('img/rbct_lstm_ts_pred.png')
st.image('img/rbct_cnn_ts_pred.png')
st.image('img/rbct_hyb_ts_pred.png')