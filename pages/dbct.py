...
df_tfidf_clean = pd.read_csv("data/distilbert_clean.csv",index_col=0)
...
df_tfidf_clean = max_score_extraction(df_tfidf_clean,"distilbert_clean")
st.title('Bitcoin hakkında atılan tweetlerin sentiment analizi')

st.subheader('distilbert Transformer ile Sentiment Analizi')
...
#show the dataset
st.write(df_tfidf_clean[["distilbert_neg_clean","distilbertdistilbert_neu_clean","distilbert_pos_clean","distilbert_clean_max_score","sentiment"]].sample(10))
...
st.dataframe(allscores[allscores.index.str.startswith("dbct")])


st.image('img/dbct_lstm_ts_pred.png')
st.image('img/dbct_cnn_ts_pred.png')
st.image('img/dbct_hyb_ts_pred.png')