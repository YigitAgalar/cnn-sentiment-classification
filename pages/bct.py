...
df_tfidf_clean = pd.read_csv("data/bert_clean.csv",index_col=0)
...
df_tfidf_clean = max_score_extraction(df_tfidf_clean,"bert_clean")
st.title('Bitcoin hakkında atılan tweetlerin sentiment analizi')

st.subheader('Bert Transformer ile Sentiment Analizi')
...
#show the dataset
st.write(df_tfidf_clean[["bert_neg_clean","bert_neu_clean","bert_pos_clean","bert_clean_max_score","sentiment"]].sample(10))
...
st.dataframe(allscores[allscores.index.str.startswith("bct")])


st.image('img/bct_lstm_ts_pred.png')
st.image('img/bct_cnn_ts_pred.png')
st.image('img/bct_hyb_ts_pred.png')